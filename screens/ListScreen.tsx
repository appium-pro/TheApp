...
const CLOUD_TYPES = {
  ...
  Azure:
    'used for deploying, designing and managing the applications through a worldwide network',
  ...
  Dropbox:
    'a refined cloud storage service used by small businesses and customers to store files or documents virtually on remote cloud servers',
  Fog: 'surface-level stratiform non-convective',
  'Google Cloud Platform':
    'uses resources such as computers, virtual machines, hard disks, etc. located at Google data centers',
  'IBM Cloud':
    'offers Iaas, PaaS, and SaaS through all the available cloud delivery models',
  Kamatera:
    'provides very low-maintenance and high-performance cloud infrastructure services',
  Nimbostratus: 'vertical/multi-level stratiform non-convective',
  Noctilucent: 'extreme-level cirriform mostly non-convective',
  'Oracle Cloud':
    'available as SaaS, PaaS, and IaaS. Oracle Cloud helps the companies in transforming their business quickness and reducing the IT Complexity',
  PhoenixNAP:
    'a global IT services provider offering secure and scalable Infrastructure-as-a-Service solutions, including private, public, and managed cloud services',
  'Polar Stratospheric': 'very high-level cirriform mostly non-convective',
  'Rackspace Cloud':
    'offers a set of cloud computing services like hosting web applications, Cloud Files, Cloud Block Storage, Cloud Backup, Databases and Cloud Servers',
  RedHat:
    'an Open Cloud technology used by IT organizations to deliver agile and flexible solutions',
  'Salesforce Cloud':
    'offers all the applications required by businesses like CRM, ERP, customer service, sales, mobile applications, and marketing',
...
type CloudType = keyof typeof CLOUD_TYPES;

const CLOUD_NAMES: CloudType[] = Object.keys(CLOUD_TYPES) as CloudType[];
...
export default function ListScreen() {
  function showPickedAlert(cloudType: CloudType) {
    const onLearnMore = () => {
      Alert.alert(cloudType, `This is a ${CLOUD_TYPES[cloudType]} cloud`);
    };
    Alert.alert(
  ...
  return (
    <View>
      ...
      <ScrollView>
        {CLOUD_NAMES.map((cloudType: CloudType) => (
          <ListItem
            key={cloudType}
            onPress={() => showPickedAlert(cloudType)}
            {...testProps(cloudType)}>
            <ListItem.Content>
